...
const Sidebar = (props) => {
  ...
  const isCollapsed =
    props.isCollapsed !== undefined ? props.isCollapsed : internalCollapsed;
  const setIsCollapsed = props.setIsCollapsed || setInternalCollapsed;
  ...
  const menuItems = [
    {
      ...
      description: "Office Staff dashboard",
      path: "/officestaff/",
    },
    {
      id: "applications",
  ...
  const isActive = (path) => {
    if (path === "/officestaff/") {
      return location.pathname === "/officestaff/";
    }
    if (path === "/officestaff/applications") {
      return location.pathname.startsWith("/officestaff/applications");
    }
    if (path === "/officestaff/help") {
      return location.pathname.startsWith("/officestaff/help");
    }
    return false;
  };
  ...
  return (
    ...
    >
      ...
      <div
        className={`p-2 border-t transition-all duration-500 ease-out flex-shrink-0 mt-12 ${
          ...
          {bottomMenuItems.map((item, index) => (
            <li key={item.id} className="relative">
              <button
                ...
                onMouseEnter={(e) => handleMouseEnter(item, e)}
                onMouseLeave={handleMouseLeave}
                className={`w-full mb-5 flex items-center rounded-xl transition-all duration-500 ease-out relative overflow-hidden group focus:outline-none focus:ring-2 focus:ring-blue-400/40 transform hover:scale-[1.02] ${
                  item.id === "logout"
                    ? isDarkMode
