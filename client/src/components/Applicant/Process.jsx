...
const ApplicationProcess = () => {
  const options = [
    { image: Submit, title: "Submit An Application", path: "/submit-application" },
    { image: Track, title: "Track Your Application", path: "/track-application" }
  ];
...
>
      ...
      <div className="flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-8 bg-opacity-70 p-8 rounded-lg shadow-[0px_20px_50px_rgba(0,0,0,0.4)] w-full max-w-5xl">
        ...
        {options.map((option, index) => (
          <Link to={option.path} key={index} className="relative w-full md:w-[30rem] h-[20rem] rounded-lg overflow-hidden group cursor-pointer">
            {/* Option image */}
            <img
              src={option.image}
              alt={option.title}
