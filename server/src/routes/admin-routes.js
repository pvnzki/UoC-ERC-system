...
router.get("/applications/:applicationId/letter", meetingController.generateLetter);//automatically generates and downloads the approval/rejection letters

// Committee Interaction Routes (3.4.x)
router.put("/applications/:applicationId/assign",committeeInteractionController.assignToCommittee); //successfully assigning applications to committees
router.put("/applications/:applicationId/outcome", committeeInteractionController.reviewCommitteeOutcome);//properly printing the outcome of ratifiedDecisions
router.post("/committees/:committeeId/email",committeeInteractionController.sendCommitteeEmail);//sending emails to committee chair by erc admin
router.post("/applications/:applicationId/email-applicant",committeeInteractionController.sendApplicantEmail);//sending emails to applicants by erc admin directly(not by the system)
